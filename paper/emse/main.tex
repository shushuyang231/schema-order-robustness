...
The disjoint decomposed studies reproduced nonzero representation effects across the four core text-mode deployments (Table 3). The supplemental panel then bounded that result: \texttt{deepseek-chat} remained positive but below 0.05, while the reasoner and TokenRhythm intervals included zero. \textbf{Answer to RQ1:} validator-equivalent serialization can change normalized black-box output distributions beyond repeated-prompt variability, but magnitude is system- and deployment-contingent; the additional endpoint evidence does not support a universal order effect.
