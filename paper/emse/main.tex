...
\author{Shengyao Sun\thanks{Corresponding author: \href{mailto:sthfornothing@sjtu.edu.cn}{sthfornothing@sjtu.edu.cn}; ORCID: \href{https://orcid.org/0009-0008-9175-8226}{0009-0008-9175-8226}}}
...
\maketitle
\begin{abstract}
\textbf{Context:} Language-model software uses JSON Schema as both an instruction artifact and a validation contract. Software tooling may reorder a Schema without changing validator behavior, yet change model behavior. \textbf{Objective:} We test whether validation-equivalent Schema serializations induce output-distribution changes beyond repeated-call variability, whether the changes exceed a prospectively specified practical screen across deployed systems, and whether provider JSON Mode attenuates them. \textbf{Method:} We derive metamorphic relations from JSON and JSON Schema semantics, issue five repeated calls per representation, and estimate record-level excess disagreement with cluster bootstrap and permutation inference. The core study contains 17,900 successful responses from two gateway aliases and official DeepSeek/Qwen endpoints, a disjoint 200-record decomposition, and a matched 100-record Qwen text/JSON-Mode ablation. Gateway results characterize observed deployments; upstream checkpoint identities were not independently verified. \textbf{Results:} Property-order and additional Schema-member-order effects exceeded 0.05 for the Sonnet gateway (0.058 and 0.077) and Qwen-Plus text deployment (0.126 and 0.123), but not for the GPT gateway (0.033 and 0.038) or DeepSeek (0.013 and 0.018). JSON-Mode effects also exceeded 0.05. A later supplemental panel added 9,000 successful responses from an institutional SJTU endpoint and a separate model aggregator: no new contrast reached 0.05; SJTU \texttt{deepseek-chat} was statistically detectable but subthreshold (0.0168 and 0.0160), while \texttt{deepseek-reasoner} and \texttt{deepseek-v4-flash} were near zero. Matched JSON-minus-text changes were +0.019 and -0.025; neither was confirmed as a material interaction. Average Schema compliance and leaf-value accuracy did not show universal degradation. \textbf{Conclusions:} Validation equivalence is an insufficient regression oracle for LM-facing Schema artifacts. We release an offline-auditable test workflow and conservative canonicalizer; distributional robustness should be tested separately from syntax and task accuracy.
\end{abstract}
...
\item \textbf{RQ2 --- Decomposed practical effects:} Do property order and additional Schema-object member order independently exceed a prospectively specified 0.05 practical screen across deployed systems?
...
Supplemental endpoint panel & 200 & SJTU \texttt{deepseek-chat}; SJTU \texttt{deepseek-reasoner}; TokenRhythm \texttt{deepseek-v4-flash} & 3 & 5 & 9,000 & post-submission, provenance-separated \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\subsection{Execution contract and model settings}
Table 2 reports the successful-response windows and endpoint-level settings that define each deployment observation. Times are UTC and are the first and last successful rows in the corresponding run log; smoke calls and transient errors are documented separately in the artifact. The gateway base URL is intentionally withheld from the public manuscript because it identifies a small private relay; its request path was OpenAI-compatible \texttt{/chat/completions}, and no gateway region field was exposed.

\begin{table}[t]
\caption{Execution windows, endpoint provenance, and request settings.}
\label{tab:markdown-2}
\centering
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrrr}
\toprule
\textbf{Interface/stage} & \textbf{Successful-response window (UTC)} & \textbf{Requested/returned model} & \textbf{Endpoint and region} & \textbf{Request contract} \\
\midrule
Sonnet gateway, primary / decomposition & 2026-07-17 10:07--16:05 / 2026-07-19 07:50--13:39 & \texttt{claude-sonnet-5} / same & Private OpenAI-compatible gateway; region not exposed & \texttt{max\_tokens=4096}; temperature, top\_p, seed omitted; text mode \\
GPT gateway, primary / decomposition & 2026-07-17 16:22--2026-07-18 04:59 / 2026-07-19 13:48--2026-07-21 09:44 & \texttt{gpt-5.5} / same & Private OpenAI-compatible gateway; region not exposed & \texttt{max\_tokens=4096}; temperature, top\_p, seed omitted; text mode \\
DeepSeek official & 2026-07-24 05:41--14:27 & \texttt{deepseek-v4-flash} / same & \texttt{https://api.deepseek.com/v1}; provider region not returned & \texttt{max\_tokens=4096}; provider-default thinking; response\_format and tools omitted \\
Qwen-Plus official text & 2026-08-01 14:46--2026-08-02 03:19 & \texttt{qwen-plus} / same & \texttt{https://\allowbreak{}dashscop\allowbreak{}e.aliyun\allowbreak{}cs.com/c\allowbreak{}ompatibl\allowbreak{}e-mode/v\allowbreak{}1}; provider region not returned & \texttt{max\_tokens=4096}; \texttt{enable\_thinking=false}; response\_format omitted \\
Qwen-Plus official JSON Mode & 2026-08-02 04:18--05:02 & \texttt{qwen-plus} / same & Same Alibaba endpoint; provider region not returned & \texttt{max\_tokens=4096}; \texttt{enable\_thinking=false}; \texttt{response\allowbreak{}\_format=\allowbreak{}\{"type":\allowbreak{}"json\_ob\allowbreak{}ject"\}} \\
SJTU Zhiyuan-1 \texttt{deepseek-chat} (recovery) & 2026-08-10 15:55--2026-08-12 04:03 & \texttt{deepseek-chat} / same & \texttt{https://\allowbreak{}models.s\allowbreak{}jtu.edu.\allowbreak{}cn/api/v\allowbreak{}1}; region not returned & \texttt{max\_tokens=4096}; provider defaults; response\_format and tools omitted; text mode \\
SJTU Zhiyuan-1 \texttt{deepseek-reasoner} & 2026-08-10 06:23--2026-08-12 04:06 & \texttt{deepseek-reasoner} / same & Same SJTU endpoint; region not returned & \texttt{max\_tokens=4096}; provider defaults; reasoning returned but not scored; response\_format and tools omitted \\
TokenRhythm \texttt{deepseek-v4-flash} & 2026-08-11 02:46--2026-08-12 04:07 & \texttt{deepseek-v4-flash} / same & Aggregator endpoint; region and fingerprint not returned & \texttt{max\_tokens=4096}; provider defaults; response\_format and tools omitted \\
\bottomrule
\end{tabular}%
}
\end{table}
Across all runs, \texttt{stream=false}, no tools were sent, and no seed was supplied. The runner used Python 3.12.13, \texttt{httpx} 0.28.1, and \texttt{jsonschema} 4.26.0. The request timeout was 240 seconds. Each request allowed at most three attempts; HTTP 429, HTTP 5xx, timeouts, and network errors used exponential backoff of 1 and 2 seconds, while other 4xx responses stopped the run. Five consecutive request errors paused execution for resumable inspection. The complete request rows, prompt hashes, returned model strings, and retry records are included in the online resource. For the supplemental panel, the analysis plan froze the six endpoint-by-contrast tests and their panel-wide Holm correction before reading the completed effect estimates; this is a post-data, pre-effect-analysis amendment, not a formal preregistration.

The exact system prompt was: `\texttt{You are \allowbreak{}a precis\allowbreak{}e inform\allowbreak{}ation ex\allowbreak{}traction\allowbreak{} system.\allowbreak{} Answer \allowbreak{}using on\allowbreak{}ly one J\allowbreak{}SON obje\allowbreak{}ct that \allowbreak{}satisfie\allowbreak{}s the su\allowbreak{}pplied J\allowbreak{}SON Sche\allowbreak{}ma. Do n\allowbreak{}ot add m\allowbreak{}arkdown,\allowbreak{} explana\allowbreak{}tions, c\allowbreak{}omments,\allowbreak{} or fiel\allowbreak{}ds not a\allowbreak{}llowed b\allowbreak{}y the sc\allowbreak{}hema. Us\allowbreak{}e only t\allowbreak{}he provi\allowbreak{}ded cont\allowbreak{}ext.'' T\allowbreak{}he exact\allowbreak{} user-te\allowbreak{}mplate, \allowbreak{}with onl\allowbreak{}y record\allowbreak{}-specifi\allowbreak{}c fields\allowbreak{} substit\allowbreak{}uted, wa\allowbreak{}s: }\texttt{\# Contex\allowbreak{}t\textbackslash{}\textbackslash{}n\{con\allowbreak{}text\}\textbackslash{}\textbackslash{}n\allowbreak{}\textbackslash{}\textbackslash{}n\# Que\allowbreak{}stion\textbackslash{}\textbackslash{}n\allowbreak{}\{questio\allowbreak{}n\}\textbackslash{}\textbackslash{}n\textbackslash{}\textbackslash{}n\allowbreak{}\# JSON S\allowbreak{}chema\textbackslash{}\textbackslash{}n\allowbreak{}\{schema\_\allowbreak{}serializ\allowbreak{}ation\}\textbackslash{}\textbackslash{}\allowbreak{}n\textbackslash{}\textbackslash{}nRetu\allowbreak{}rn only \allowbreak{}the JSON\allowbreak{} object.\allowbreak{}'' This \allowbreak{}template\allowbreak{} was ide\allowbreak{}ntical a\allowbreak{}cross re\allowbreak{}presenta\allowbreak{}tions; o\allowbreak{}nly }\{schema\_serialization\}` changed.
...
Table 3 reports the central decomposed results and the post-submission endpoint panel. Both effects crossed the 0.05 rule in the Sonnet gateway and official Qwen-Plus text deployments. The three supplemental deployments added no practical confirmation: SJTU \texttt{deepseek-chat} showed statistically detectable but subthreshold effects, whereas SJTU \texttt{deepseek-reasoner} and TokenRhythm were near zero. Practical magnitude varies across the tested deployments.

\begin{table}[t]
\caption{Decomposed text-mode effects, supplemental endpoint results, and deployment-level decisions.}
\label{tab:markdown-3}
\centering
\resizebox{\linewidth}{!}{%
\begin{tabular}{lrrrrr}
\toprule
\textbf{System} & \textbf{Records} & \textbf{Property-order excess (95\% CI)} & \textbf{Additional-member excess (95\% CI)} & \textbf{Schema pass} & \textbf{Decision} \\
\midrule
...
SJTU Zhiyuan-1 \texttt{deepseek-chat} (recovery) & 200 & 0.0168 [0.0023, 0.0331] & 0.0160 [0.0036, 0.0298] & 98.83\% & detectable, subthreshold \\
SJTU Zhiyuan-1 \texttt{deepseek-reasoner} & 200 & -0.0005 [-0.0111, 0.0114] & 0.0028 [-0.0070, 0.0130] & 99.73\% & neither confirmed \\
TokenRhythm \texttt{deepseek-v4-flash} & 200 & 0.0039 [-0.0064, 0.0163] & -0.0027 [-0.0112, 0.0064] & 99.67\% & neither confirmed \\
\bottomrule
\end{tabular}%
}
\end{table}
All Sonnet, GPT, and Qwen adjusted p-values in Table 3 are 0.0004. DeepSeek adjusted p-values are 0.0132 and 0.0016. For the supplemental panel, local two-contrast Holm p-values are 0.0052 for both SJTU \texttt{deepseek-chat} contrasts, 0.6151 for both SJTU \texttt{deepseek-reasoner} contrasts, and 0.4759/0.7101 for TokenRhythm. After Holm correction across all six supplemental tests, the corresponding values are 0.0156/0.0156, 1.0000/0.9518, and 0.9518/1.0000. Statistical detectability is therefore not the same as the frozen practical decision, and panel-wide correction does not turn any supplemental estimate into a threshold crossing.
...
\caption{Decomposed order effects across the four core text-mode deployments; the supplemental endpoint panel is tabulated in Table 3.}
...
The disjoint decomposed studies reproduced nonzero representation effects across the four core text-mode deployments (Table 3). The supplemental panel then bounded that result: \texttt{deepseek-chat} remained positive but below 0.05, while the reasoner and TokenRhythm intervals included zero. \textbf{Answer to RQ1:} validator-equivalent serialization can change normalized black-box output distributions beyond repeated-prompt variability, with evidence spanning clear practical effects, subthreshold positive estimates, and intervals compatible with zero.
...
SJTU \texttt{deepseek-chat} & -0.0225 [-0.0404, -0.0067] & +0.0131 [-0.0022, 0.0292] \\
SJTU \texttt{deepseek-reasoner} & +0.0065 [-0.0085, 0.0220] & -0.0080 [-0.0221, 0.0057] \\
TokenRhythm \texttt{deepseek-v4-flash} & -0.0080 [-0.0204, 0.0035] & -0.0006 [-0.0154, 0.0140] \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Schema pass rates remain high in every core run: 99.70\% Sonnet, 99.33\% GPT, 99.63\% DeepSeek, 99.33\% Qwen text, and 98.73\% Qwen JSON Mode. The supplemental panel rates were 98.83\% for SJTU chat, 99.73\% for SJTU reasoner, and 99.67\% for TokenRhythm. The slightly lower JSON-Mode and chat rates are compatible with interface-specific output behavior, not evidence of a universal quality loss. \textbf{Answer to RQ3:} distributional instability and average task degradation are empirically distinct. The former is repeatedly observed; the latter is not universal.
...
\subsection{Practical magnitude varies across deployments}
