...
We study five serialization transformations in an initial 100-record experiment and then isolate two interpretable contrasts on a disjoint 200-record panel: property order, and additional Schema-object member order while property order is held fixed. We retain practical confirmations and subthreshold replications across deployed interfaces rather than selecting only systems that support the hypothesis. We further test the same 100 Qwen-Plus records in provider JSON Mode. This mode requests a JSON object but is not strict JSON-Schema-constrained decoding; it lets us ask whether syntax-oriented interface support materially changes the representation effect.
...
This paper makes five contributions. First, it turns validation equivalence into a metamorphic relation for testing LM-facing Schema artifacts. Second, it provides a stochasticity-adjusted excess-disagreement estimator with record-cluster inference and a prospectively specified practical screen. Third, it reports practical confirmations and subthreshold replications across gateway aliases and official endpoints, including 17,900 successful core responses and a provenance-separated 9,000-response supplemental endpoint panel. Fourth, it supplies a matched text-versus-JSON-Mode boundary experiment showing that syntax-oriented JSON Mode did not demonstrate attenuation in this Qwen deployment. Fifth, it provides a conservative canonicalizer and a continuous-integration (CI) regression workflow that remove the tested representation degree of freedom by construction.

The durable contribution is the specification-derived test and mitigation workflow. Deployment estimates characterize the named interfaces during the recorded runs and should be remeasured after provider, model, SDK, prompt-wrapper, or Schema-generator changes.

Our estimand is deployment-level distributional sensitivity under validator-equivalent Schema serialization; mechanism identification and strict constrained decoding are outside the present design. We test deployed black-box systems at recorded dates and evaluate distributional robustness separately from syntax and task quality.
...
The project evolved through separately frozen stages. Table 1 distinguishes the initial experiment, disjoint decomposition, official-endpoint replications, interface ablation, and a later endpoint panel. The first five rows comprise the 17,900 successful responses used in the core evidence. The final row is a post-submission supplemental panel: it was analyzed under a separate completion and six-test Holm audit, is not pooled with the core evidence, and does not change the frozen threshold or record selection. A separate 40-record fixed-snapshot Qwen pilot produced 600 responses but was stopped for cost/entitlement reasons and is neither pooled nor used for a formal result.
...
| Supplemental endpoint panel | 200 | SJTU `deepseek-chat`; SJTU `deepseek-reasoner`; TokenRhythm `deepseek-v4-flash` | 3 | 5 | 9,000 | post-submission, provenance-separated |
...
The post-submission endpoint panel contains 9,000 successful responses: 3,000 each from SJTU Zhiyuan-1 `deepseek-chat`, SJTU Zhiyuan-1 `deepseek-reasoner`, and the TokenRhythm `deepseek-v4-flash` alias. The SJTU service returned stable aliases and the same `vllm-0.21.0-tp8-ep-d873796e` system fingerprint for both deployments; the TokenRhythm responses exposed no system fingerprint. The SJTU endpoint is an institutional service attributed to Shanghai Jiao Tong University's Zhiyuan-1 computing platform, whose backend metadata indicate Huawei Ascend 910B NPU infrastructure. We make no hardware-causal claim: provider, model alias, middleware, and hardware are confounded. TokenRhythm is treated as an observed aggregator deployment, and its upstream checkpoint identity is not independently verified. These runs were completed after the submission and are reported as supplemental evidence rather than as new core model-family replications.

The supplemental panel had 28 retained top-level transport-error rows (6 for `deepseek-chat`, 15 for `deepseek-reasoner`, and 7 for TokenRhythm); all 9,000 successful request keys were unique and completed after resumption. An offline completion audit found no duplicate successful keys, returned-model drift, or smoke-hash drift. The initial runner stopped after a transient-error streak; the same request keys were then resumed, and no effect analysis was performed until the panel was complete. This makes the completion audit part of the evidence, not a hidden exclusion rule.
...
| SJTU Zhiyuan-1 `deepseek-chat` (recovery) | 2026-08-10 15:55–2026-08-12 04:03 | `deepseek-chat` / same | `https://models.sjtu.edu.cn/api/v1`; region not returned | `max_tokens=4096`; provider defaults; response_format and tools omitted; text mode |
| SJTU Zhiyuan-1 `deepseek-reasoner` | 2026-08-10 06:23–2026-08-12 04:06 | `deepseek-reasoner` / same | Same SJTU endpoint; region not returned | `max_tokens=4096`; provider defaults; reasoning returned but not scored; response_format and tools omitted |
| TokenRhythm `deepseek-v4-flash` | 2026-08-11 02:46–2026-08-12 04:07 | `deepseek-v4-flash` / same | Aggregator endpoint; region and fingerprint not returned | `max_tokens=4096`; provider defaults; response_format and tools omitted |

Across all runs, `stream=false`, no tools were sent, and no seed was supplied. The runner used Python 3.12.13, `httpx` 0.28.1, and `jsonschema` 4.26.0. The request timeout was 240 seconds. Each request allowed at most three attempts; HTTP 429, HTTP 5xx, timeouts, and network errors used exponential backoff of 1 and 2 seconds, while other 4xx responses stopped the run. Five consecutive request errors paused execution for resumable inspection. The complete request rows, prompt hashes, returned model strings, and retry records are included in the online resource. For the supplemental panel, the analysis plan froze the six endpoint-by-contrast tests and their panel-wide Holm correction before reading the completed effect estimates; this is a post-data, pre-effect-analysis amendment, not a formal preregistration.
...
Table 3 reports the central decomposed results and the post-submission endpoint panel. Both effects crossed the 0.05 rule in the Sonnet gateway and official Qwen-Plus text deployments. The three supplemental deployments added no practical confirmation: SJTU `deepseek-chat` showed statistically detectable but subthreshold effects, whereas SJTU `deepseek-reasoner` and TokenRhythm were near zero. Practical magnitude varies across the tested deployments.

| System | Records | Property-order excess (95% CI) | Additional-member excess (95% CI) | Schema pass | Decision |
|---|---:|---:|---:|---:|---|
...
| SJTU Zhiyuan-1 `deepseek-chat` (recovery) | 200 | 0.0168 [0.0023, 0.0331] | 0.0160 [0.0036, 0.0298] | 98.83% | detectable, subthreshold |
| SJTU Zhiyuan-1 `deepseek-reasoner` | 200 | -0.0005 [-0.0111, 0.0114] | 0.0028 [-0.0070, 0.0130] | 99.73% | neither confirmed |
| TokenRhythm `deepseek-v4-flash` | 200 | 0.0039 [-0.0064, 0.0163] | -0.0027 [-0.0112, 0.0064] | 99.67% | neither confirmed |

All Sonnet, GPT, and Qwen adjusted p-values in Table 3 are 0.0004. DeepSeek adjusted p-values are 0.0132 and 0.0016. For the supplemental panel, local two-contrast Holm p-values are 0.0052 for both SJTU `deepseek-chat` contrasts, 0.6151 for both SJTU `deepseek-reasoner` contrasts, and 0.4759/0.7101 for TokenRhythm. After Holm correction across all six supplemental tests, the corresponding values are 0.0156/0.0156, 1.0000/0.9518, and 0.9518/1.0000. Statistical detectability is therefore not the same as the frozen practical decision, and panel-wide correction does not turn any supplemental estimate into a threshold crossing.

![Figure 2. Primary five-way effects for the two gateway deployments.](figures/figure2_forest.png)

![Figure 3. Decomposed order effects across the four core text-mode deployments; the supplemental endpoint panel is tabulated in Table 3.](figures/figure3_decomposed_confirmation.png)
...
The same 100 records on the GPT gateway showed smaller effects: 0.0389 for property reversal, 0.0109 for required reversal, 0.0287 for keyword reversal, and 0.0015 for descriptions-first. The contrast between the two gateway reports was exploratory, and paired Sonnet-minus-GPT comparisons did not support a deployment ranking. The study therefore reports one practical confirmation and one subthreshold replication, with both estimates retained.
...
The disjoint decomposed studies reproduced nonzero representation effects across the four core text-mode deployments (Table 3). The supplemental panel then bounded that result: `deepseek-chat` remained positive but below 0.05, while the reasoner and TokenRhythm intervals included zero. **Answer to RQ1:** validator-equivalent serialization can change normalized black-box output distributions beyond repeated-prompt variability, with evidence spanning clear practical effects, subthreshold positive estimates, and intervals compatible with zero.
...
GPT produced 0.0329 and 0.0380. DeepSeek produced 0.0133 and 0.0182. Their positive intervals and adjusted p-values indicate detectable shifts, but none passed the practical magnitude screen. The supplemental SJTU chat deployment produced 0.0168 and 0.0160, also below the screen; the reasoner and TokenRhythm estimates were close to zero with intervals spanning zero. These subthreshold estimates are retained rather than collapsed into a null category, showing that statistical detectability and practical magnitude are distinct.

The decomposition changes the interpretation of the original keyword-reversal result. It shows that a representation shift remains after property order is held fixed; the contrast aggregates several remaining Schema-member positions rather than isolating one keyword. **Answer to RQ2:** both decomposed order dimensions exceeded the prospectively specified screen in two core deployments and remained below it in the other two core deployments and all three supplemental deployments. Practical magnitude therefore depends on the tested deployment.
...
| SJTU `deepseek-chat` | -0.0225 [-0.0404, -0.0067] | +0.0131 [-0.0022, 0.0292] |
| SJTU `deepseek-reasoner` | +0.0065 [-0.0085, 0.0220] | -0.0080 [-0.0221, 0.0057] |
| TokenRhythm `deepseek-v4-flash` | -0.0080 [-0.0204, 0.0035] | -0.0006 [-0.0154, 0.0140] |

The GPT additional-member contrast is the one secondary exception: -0.0248, with Holm (p=0.0208). Its magnitude remains below the earlier 0.03 engineering context, it was not a primary headline outcome, and it did not reproduce in the other deployments. The SJTU chat property-order accuracy difference is another statistically detectable but sub-0.03 secondary signal (-0.0225); the reasoner and TokenRhythm intervals include zero. We report these results rather than erase them, but they cannot support a universal accuracy claim.

Schema pass rates remain high in every core run: 99.70% Sonnet, 99.33% GPT, 99.63% DeepSeek, 99.33% Qwen text, and 98.73% Qwen JSON Mode. The supplemental panel rates were 98.83% for SJTU chat, 99.73% for SJTU reasoner, and 99.67% for TokenRhythm. The slightly lower JSON-Mode and chat rates are compatible with interface-specific output behavior, not evidence of a universal quality loss. **Answer to RQ3:** distributional instability and average task degradation are empirically distinct. The former is repeatedly observed; the latter is not universal.
...
The interaction estimates are +0.0191 and -0.0249, both below the 0.03 material-interaction threshold with intervals spanning zero. The data are compatible with modest attenuation or amplification but do not resolve a material mode interaction. The engineering boundary is therefore: **in this Qwen-Plus deployment, syntax-oriented JSON Mode did not demonstrate material attenuation of sensitivity to validation-equivalent Schema serialization.** Strict JSON-Schema-constrained decoding is outside the present estimand. This answers RQ4.
...
### 6.2 Practical magnitude varies across deployments

Sonnet and Qwen crossed the study's practical screen. GPT and DeepSeek produced positive subthreshold estimates, while the post-submission endpoint panel added no practical confirmation: SJTU chat was 0.0168/0.0160, and the reasoner and TokenRhythm estimates were near zero. The heterogeneity bounds the general claim and argues against a universal field-order heuristic. The safer rule is empirical: test the deployed combination of model, provider, prompt wrapper, mode, task mix, and Schema generator.
...
The tested gateway labels are deployment identifiers. Prompt wrapping, routing, caching, and backend changes are inseparable from the observed behavior, so the estimand remains deployment-level rather than an intrinsic Claude-versus-GPT comparison.
...
The provenance boundary is defined in Section 3.4: gateway aliases, institutional endpoints, and aggregator routes identify deployments rather than fixed checkpoints. The supplemental SJTU runs identify returned aliases and a stable vLLM fingerprint, but model, middleware, and Ascend-910B hardware effects remain confounded; the TokenRhythm aggregator exposes no fingerprint. The chat arm was resumed after an initial transient-error pause, and all 9,000 successful keys were audited before analysis. The Qwen text design used a staged futility rule; continuation followed the frozen criterion, and the full report retains the interim records. The separate fixed-snapshot Qwen pilot was inspected, then excluded from the resource study and never pooled.
...
The benchmark covers medium and hard text-to-JSON tasks, not every Schema construct, domain, language, prompt, temperature, or agent architecture. The disjoint follow-up further conditions on transformations being nontrivial. Eight deployed interfaces are not a representative sample of all LMs; two core interfaces are gateway aliases, one supplemental interface is an aggregator, and the SJTU deployments share an institutional backend fingerprint. The supplemental panel was completed after submission and is not an independent model-family sample. No fixed open-weight checkpoint was tested. Replication with versioned open models, production schemas, other languages, and full constrained-decoding engines is needed.
...
Validation-equivalent JSON Schema serializations can change the behavior of an LM-facing component even when the validator-visible contract is unchanged. Across 17,900 successful core responses, property order and additional Schema-member order produced effects beyond repeated-call variability. Practical magnitude varied by deployment: both contrasts crossed 0.05 for the Sonnet gateway and official Qwen deployments, while GPT and DeepSeek yielded positive subthreshold estimates. A separate 9,000-response endpoint panel produced no additional practical confirmation: SJTU chat was detectable but subthreshold, and SJTU reasoner and TokenRhythm were near zero after panel-wide Holm correction. Gateway aliases and aggregators are deployment-level observations, and the SJTU panel does not isolate hardware effects. Average quality metrics remained separate from distributional sensitivity rather than moving uniformly with it.
