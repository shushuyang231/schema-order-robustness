...
The disjoint decomposed studies reproduced nonzero representation effects across the four core text-mode deployments (Table 3). The supplemental panel then bounded that result: `deepseek-chat` remained positive but below 0.05, while the reasoner and TokenRhythm intervals included zero. **Answer to RQ1:** validator-equivalent serialization can change normalized black-box output distributions beyond repeated-prompt variability, but magnitude is system- and deployment-contingent; the additional endpoint evidence does not support a universal order effect.
...
Sonnet and Qwen crossed the study's practical screen; GPT and DeepSeek did not. The post-submission endpoint panel added no practical confirmation: SJTU chat was positive but only 0.0168/0.0160, and the reasoner and TokenRhythm estimates were near zero. This heterogeneity is not a defect to conceal. It bounds the claim and argues against universal folklore such as “always put important fields first.” The safer rule is empirical: the deployed combination of model, provider, prompt wrapper, mode, task mix, and Schema generator should be tested.
...
Validation-equivalent JSON Schema serializations are not necessarily behaviorally interchangeable when the Schema also acts as an LM instruction artifact. Across 17,900 successful core responses, property order and additional Schema-member order produced effects beyond repeated-call variability. Their practical magnitude was deployment-contingent: both crossed 0.05 for the Sonnet gateway and official Qwen deployments, while the GPT gateway and official DeepSeek deployment provided detectable but smaller negative practical replications. A separate 9,000-response endpoint panel produced no additional practical confirmation: SJTU chat was detectable but subthreshold, and SJTU reasoner and TokenRhythm were near zero after panel-wide Holm correction. Gateway aliases and aggregators characterize tested deployments rather than independently verified upstream checkpoints; the SJTU panel does not support a hardware-causal claim. The shifts did not establish universal average accuracy harm.
